...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def iter(tweetSet: TweetSet, list: TweetList): TweetList = {
      if (tweetSet.isInstanceOf[Empty]) {
        list
      }
      else {
        val most = tweetSet.mostRetweeted
        new Cons(most, iter(tweetSet.remove(most), list))
      }
    }
    iter(this, Nil)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val leftMax = if (!left.isInstanceOf[Empty]) left.mostRetweeted else new Tweet("", "", Int.MinValue)
    val rightMax = if (!right.isInstanceOf[Empty]) right.mostRetweeted else new Tweet("", "", Int.MinValue)
    if (leftMax.retweets > rightMax.retweets) {
      if (leftMax.retweets > elem.retweets) leftMax
      else elem
    } else {
      if (rightMax.retweets > elem.retweets) rightMax
      else elem
    }
  }
